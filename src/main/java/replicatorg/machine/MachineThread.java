	...
	void runCommand(MachineCommand command) {		
		...
		case CONNECT:
			if (state.getState() == MachineState.State.NOT_ATTACHED) {
				...
				if (!connected) {
					...
					if (driver.isInitialized()) {
						readName();
						setState(new MachineState(MachineState.State.CONNECTED), readyMessage());
					} else {
		...
		case RESET:
			if (state.isConnected()) {
				driver.reset();
				readName();
				setState(new MachineState(MachineState.State.CONNECTED),
						readyMessage());
			}
			break;
		case BUILD_DIRECT:
...
//				System.out.println("post-");
		...
		case STOP_MOTION:
			...
			if (state.getState() == MachineState.State.BUILDING) {
				setState(new MachineState(MachineState.State.FINISHED),
						readyMessage());
			}
		...
		while (true) {
			...
			if(state.isConnected())
			{
				...
				if ( pollingTimer.elapsed() ) {
					...
					//check for temp
					boolean checkTempDuringBuild = true;
