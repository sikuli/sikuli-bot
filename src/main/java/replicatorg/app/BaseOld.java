...
public class BaseOld {
	...
	*/
	...
	public static void setLogFile(String path) {
		boolean useLogFile = BaseOld.preferences.getBoolean("replicatorg.useLogFile",false);
	...
	{	
		String levelName = BaseOld.preferences.get("replicatorg.debuglevel", Level.INFO.getName());
		Level l = Level.parse(levelName);
		logger.setLevel(l);

		String logPath = BaseOld.preferences.get("replicatorg.logpath", "");
		setLogFile(logPath);
	}
	/**
	...
	static Preferences getUserPreferences() {
		Preferences prefs = Preferences.userNodeForPackage(BaseOld.class);
	...
	static private void moveAll(File source, File target) {
		try {
			...
			BaseOld.logger.severe(e.getMessage());
		}
	}
	...
	static public void resetPreferences() {
		try {
			BaseOld.preferences.removeNode();
			BaseOld.preferences.flush();
			preferences = getUserPreferences();
	...
	static public String getToolsPath() {
	    ...
	    if (toolsDir == null || (toolsDir.length() == 0)) {
		    File appDir = BaseOld.getApplicationDirectory();
	...
	static public File getUserDirectory() {
		...
		if (!dir.exists()) {
			...
			if( ! dir.exists() )  { // we failed to create our user dir. Log the failure, try to continue
				BaseOld.logger.severe("We could not create a user directory at: "+ path );
				return null; 
			}
		}
	...
	static public File getUserFile(String path, boolean autoCopy) {
		if (path.contains("..")) {
			BaseOld.logger.info("Attempted to access parent directory in "+path+", skipping");
			return null;
		}
		...
		if (autoCopy && !f.exists()) {
			...
			if (original.exists()) {
				try {
					BaseOld.copyFile(original,f);
				} catch (IOException ioe) {
					BaseOld.logger.log(Level.SEVERE,"Couldn't copy "+path+" to your local .replicatorG directory",f);
				}
			}
		}
		return f;
    ...
    */
	...
	static public void main(String args[]) {
		...
		for (int i=0;i<args.length;i++) {
			if (args[i].equals("--alternate-prefs")) {
				...
				BaseOld.openedAtStartup = args[i];
			}
		}
		...
		MRJOpenDocumentHandler startupOpen = new MRJOpenDocumentHandler() {
			public void handleOpenFile(File file) {
				...
				if (BaseOld.openedAtStartup == null) {
					BaseOld.openedAtStartup = file.getAbsolutePath();
				}
			}
		};
	 ...
	 */
	public BaseOld(boolean cleanPrefs) {
		if (cleanPrefs) {
			resetPreferences();
		}
...
//		    private TrayIcon trayIcon;
			...
			public void run() {
				...
				String machine = BaseOld.preferences.get("machine.name", null);
				if(machine == null)
					BaseOld.preferences.put("machine.name", "The Replicator Dual");
                ...
                */
				...
				boolean autoconnect = BaseOld.preferences.getBoolean("replicatorg.autoconnect",true);
		...
		try {
			if (BaseOld.isWindows()) {
				...
				if (launcher != null) {
					...
					BaseOld.logger.warning("Unspecified platform, no launcher available.");
				}
			}
	...
	static public boolean openFolderAvailable() {
		if (BaseOld.isWindows() || BaseOld.isMacOS())
			return true;
...
//	}
	...
	static public String getContents(String what) {
		File appBase = 	BaseOld.getApplicationDirectory();
	...
	static public BufferedImage getImage(String name, Component who) {
	    ...
	    try {
			...
			BaseOld.logger.log(Level.FINE, "Could not load image: "+name, e);
		} catch (IOException ioe) {
			BaseOld.logger.log(Level.FINE, "Could not load image: "+name, ioe);
		} catch (IllegalArgumentException iae) {
			BaseOld.logger.log(Level.FINE, "Could not load image: "+name, iae);
		}
		return image;
	}
	...
	static public String loadFile(File file) throws IOException {
		BaseOld.logger.info("Load file : "+file.getAbsolutePath());
	...
	static public void saveFile(String str, File file) throws IOException {
		BaseOld.logger.info("Saving as "+file.getCanonicalPath());
